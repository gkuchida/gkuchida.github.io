...
export class Home implements OnInit, AfterViewInit {
 ...
 ngOnInit(): void {
  ...
  this.slides = this.chunkArray(this.produtos, 3);
  this.inspiracoesSlides = this.chunkArray(this.inspiracoes, 3);
 ...
 private updateNovidadesSlides() {
  const novidades = modelosProntas.filter(p => p.isNovidade);
  this.slidesNovidades = this.chunkArray(novidades, this.novidadesGroupSize) as ProdutoPronto[][];
 }
